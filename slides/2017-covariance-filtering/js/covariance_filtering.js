...
if(!loadSVG) {
   function loadSVG(uri, elem, call) {
...
const loadElementFromSVG = function(uri, svgid, elem, call) {
    ...
    if(base == undefined) {
        ...
        url = getData("general", "baseurl") + uri
    }
...
var createFilter = function(snap, x0, y0, w, h, sigma, orient, style) {
    ...
    var pathB = snap.path(curveB).attr({fillOpacity: 0, stroke: "#24ab00", strokeWidth: 2});
    ...
    var textA = snap.text(x0+w, y0-h-30, "Kernel").attr({fill: "#ff0000", textAnchor: "end", fontSize: "0.8em"});
    var textA = snap.text(x0+w, y0-h-5, "Material").attr({fill: "#24ab00", textAnchor: "end", fontSize: "0.8em"});
    var textA = snap.text(x0+w, y0-h+20, "Antaliased material").attr({fill: "#0000ff", textAnchor: "end", fontSize: "0.8em"});
...
var antialiasingAppearance06Step00 = function(s) {
    ...
    var cov   = CreateCovariance(s, frame, Snap.matrix(0.9, 0, 0, 0.1, 0, 0));
    var text  = s.text(cx, cy+w/2+30, "Covariance").attr({fontSize: "0.6em", textAnchor: "middle"});
    frame.attr({ class: "covariance", opacity: 0});
    text.attr({ class: "covariance", opacity: 0});
    ...
    var text = s.paper.text(x0+w+50, y0+7, "kernel").attr({fill: "#ff0000", fontSize: "0.8em", textAnchor: "middle"});
