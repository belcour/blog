...
function ProjectiveSpace(snap, NB_ELEMS = 6, COLOR = "#AAF", SIZE = 256, POS_X=SIZE, POS_Y=SIZE, FONTSIZE='4', STROKEWIDTH=2) {
    var g = snap.g();
    ...
    for(theta=dtheta; theta<0.5*Math.PI-dtheta; theta+=dtheta) {
        ...
        var c = snap.circle(POS_X, POS_Y, radius).attr({fillOpacity: 0, stroke: COLOR, strokeWidth: STROKEWIDTH, opacity: 0.2});
        var t = snap.text(POS_X/*-18*/, POS_Y-radius+0.5*FONTSIZE, Math.round(degree) + '°').attr({fill: COLOR, fontSize: FONTSIZE+'px', textAnchor: 'middle'});
        g.add(c);
        g.add(t);
